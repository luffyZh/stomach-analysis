...
export default {
  ...
  data(){
    ...
    symptomArr:[//临床症状
        ...
        {key:'symptom2',desc:'内镜下表现为粘膜红白相间，以白为主，可透见粘膜下血管网，病理提示肠上皮化生，属于癌前病变'},
        {key:'symptom3',desc:'内镜表现为食管粘膜条状糜烂及溃疡，临床表现为反酸烧心，常见并发症为出血，Barrett食管及癌变'},
        {key:'symptom4',desc:'无明显及特异临床表现'}
    ],
    programmeArr:[//治疗方案
       {key:'programme1',desc:'抑酸药+粘膜保护剂'},
        {key:'programme2',desc:'内镜下切除，切除后应用抑酸药及粘膜保护剂治疗'},
        {key:'programme3',desc:'注意易消化饮食，避免饮酒辛辣刺激及质硬食物，去除诱因（如根除HP治疗），给予保护粘膜对症治疗'},
  ...
  if(obj.reviewTime.key===resultObj.reviewTime){
    ...
    if(this.allScore===100){
      ...
      this.endDesc="万丈高楼平地起,一砖一瓦皆根基。虽然已经及格，但还需要继续努力弥补漏洞哦。"
    }else if(this.allScore<64){
      this.endDesc="认真做好每一件事情，写每一个字，看每一本书，做每一道题，静下心来耐心做事的你，一定会比现在更好，加油!"
    }
    //表格描述
    //位置定位
